...
func (l *LogProcess) Process() {
	...
	*/
	...
	for v := range l.rc {
		...
		if len(ret) != 14 {
			log.Println("FindStringSubmatch failure:", string(v))
			continue
		}
...
func (w *WriteToInfluxDB) Writer(wc chan *Message) {
	/** 写入模块
	* docker run -d -p 8086:8086 -p 8083:8083 -v $PWD:/var/lib/influxdb -e INFLUXDB_ADMIN_ENABLED=true influxdb influxdb
	* docker exec -it influxdb /bin/bash
	* > CREATE DATABASE imooc  ## 创建数据库imooc
	* > CREATE USER imooc WITH PASSWORD 'imoocpass' ## 创建用户和设置密码
	* > GRANT ALL PRIVILEGES ON imooc TO imooc ## 授权数据库给指定用户
	* > CREATE RETENTION POLICY "imoocretention" ON "imooc" DURATION 30d REPLICATION 1 DEFAULT ## 创建默认的数据保留策略，设置保存时间30天，副本为1
	 */
